      ...
      <div className="container py-5">
        ...
        <div className="row g-4 justify-content-center">
          {blogPosts.map((post, index) => (
            <div className="col-md-6 col-lg-6 col-xl-4 wow fadeInUp" data-wow-delay={`${0.1 * (index + 1)}s`} key={index}>
              <div className="blog-item rounded">
                <div className="blog-img">
                  <img src={post.image} className="img-fluid w-100" alt="Image" />
                </div>
                <div className="blog-centent p-4">
                  <div className="d-flex justify-content-between mb-4">
                    <p className="mb-0 text-muted">
                      <i className="fa fa-calendar-alt text-primary"></i> {post.date}
                    </p>
                    <a href={post.link} className="text-muted">
                      <span className="fa fa-comments text-primary"></span> {post.comments} Comments
                    </a>
                  </div>
                  <a href={post.link} className="h4">{post.title}</a>
                  <p className="my-4">{post.description}</p>
                  <a href={post.link} className="btn btn-primary rounded-pill text-white py-2 px-4 mb-1">
                    Read More
                  </a>
                </div>
              </div>
